...
export default function SingleSpot() {
    ...
    useEffect(() => {
        dispatch(spotActions.loadSingleSpot(id));
    }, [dispatch, id]);
    ...
    if(!spot.SpotImages) { imgArr = ['url', 'url', 'url', 'url', 'url']; }


    // Object.values(targetSpot.SpotImages).map(img=>imgArr.push(img));
    // for(let i = 0; i < 5; i++){
    ...
    //                 img.preview===true;
    //                 return img.url
    //             }) ||*/ `../../../images/preview${targetSpot.id}.jpg`} />
    //         </div>
